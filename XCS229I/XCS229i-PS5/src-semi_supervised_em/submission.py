...
def main(is_semi_supervised, trial_num):
    ...
    for i in range(K):
        if i!=K-1:
            x_temp = x[idx[i*group: (i+1)*group], :]
        else:
            x_temp = x[idx[i*group: n], :]
...
def run_em(x, w, phi, mu, sigma, max_iter=1000):
    ...
    while it < max_iter and (prev_ll is None or np.abs(ll - prev_ll) >= eps):
        ...
        it, ll, prev_ll = compute_log_likelihood(it, ll, mu, phi, prev_ll, sigma, w, x)
...
def compute_log_likelihood(it, ll, mu, phi, prev_ll, sigma, w, x):
    it += 1
    prev_ll = ll
    p_z_given_x = np.zeros(w.shape)
    for i in range(K):
        p_z_given_x[:, i] = np.exp(-0.5 * ((x - mu[i]).dot(np.linalg.inv(sigma[i])) * (x - mu[i])).sum(axis=1)) / (
                np.linalg.det(sigma[i]) ** 0.5) * phi[i]
    ll = np.sum(np.log(p_z_given_x))
    return it, ll, prev_ll

def e_step(mu, phi, sigma, w, x):
    for i in range(K):
