...
def get_initial_params(input_size, num_hidden, num_output):
    ...
    # *** START CODE HERE ***
    dict = {'W1': np.random.normal(size=(input_size, num_hidden)),
...
def forward_prop(data, labels, params):
    ...
    # activation function
    activation = sigmoid(data.dot(W1) + b1)

    # output
    output = softmax(activation.dot(W2) + b2)

    # loss
    loss = np.sum(-labels * np.log(output)) / data.shape[0]

    return activation, output, loss
    # *** END CODE HERE ***
...
def backward_prop(data, labels, params, forward_prop_func):
    ...
    """
    # *** START CODE HERE ***
    act, output, _ = forward_prop_func(data, labels, params)

    # gradient computation
    batch_size = data.shape[0]
    diff = output - labels
    # W2_grad = act.T.dot(diff) / batch_size
    W2_grad = np.dot(act.T, diff) / batch_size

    # b2_grad = np.sum(diff, axis=0) / batch_size
    b2_grad = diff.sum(0)/batch_size

    # W1_grad = data.T.dot(diff.dot(params['W2'].T) * act * (1 - act))
    # / batch_size
    d1 = np.dot(diff, params['W2'].T) * act
    d2 = d1 * (1-act)
    W1_grad = np.dot(data.T, d2) / batch_size
    # b1_grad = np.sum(diff.dot(params['W2'].T) * act * (1 - act), axis=0)
    # / batch_size
    b1_grad = d2.sum(0) / batch_size
...
def gradient_descent_epoch(train_data, train_labels, learning_rate, batch_size, params, forward_prop_func, backward_prop_func):
    ...
    # *** START CODE HERE ***
    (num_train_example, _) = train_data.shape
    for i in range(num_train_example // batch_size):
        grads = backward_prop_func(train_data[i * batch_size:i * batch_size + batch_size, :],
